...
export default function DetailExpander({ onUpdate, resetKey }: DetailExpanderProps) {
        ...
        <div className="flex flex-wrap gap-1.5">
          {LOCATIONS.map((l) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${location === l ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-accent"
                }`}
            >{l}</button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {TRIGGERS.map((t) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${trigger === t ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-accent"
                }`}
            >{t}</button>
          ))}
        </div>
        ...
        <div className="flex gap-2">
          {MOODS.map((m) => (
            <button
              ...
              className={`text-2xl p-1.5 rounded-lg transition-all ${mood === m ? "bg-primary/10 scale-110" : "hover:bg-secondary"
                }`}
            >{m}</button>
          ))}
        </div>
