...
const APP_SUBPATH = '/alcohol_craving';
...
apiRouter.get('/craving', async (req, res) => {
...
apiRouter.post('/craving', async (req, res) => {
  const user_id = req.headers['x-user-id'] as string;
...
apiRouter.delete('/craving/:id', async (req, res) => {
  const user_id = req.headers['x-user-id'] as string;
  const { id } = req.params;
  if (!user_id) return res.status(401).send('Unauthorized');
  try {
...
// Mount API at BOTH /api and /alcohol_craving/api
