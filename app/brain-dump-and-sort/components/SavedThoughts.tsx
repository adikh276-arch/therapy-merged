...
export const SavedThoughts = ({ sessions, onBack, onDelete }: Props) => {
  ...
  const bucketLabels: Record<string, { emoji: string; label: string; colorClass: string }> = {
    action: { emoji: "🟢", label: t("brain_dump_and_sort.action_needed"), colorClass: "bg-calm-green/20 border-calm-green" },
    later: { emoji: "🟡", label: t("brain_dump_and_sort.do_later"), colorClass: "bg-calm-yellow/20 border-calm-yellow" },
    letgo: { emoji: "🔵", label: t("brain_dump_and_sort.let_it_go"), colorClass: "bg-calm-blue/20 border-calm-blue" },
  };
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-8 min-h-screen flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">{t("brain_dump_and_sort.history")}</h1>
          <p className="text-muted-foreground text-sm">{t("brain_dump_and_sort.no_sessions").split('.')[0]}</p>
        </div>
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-muted-foreground text-center">
            {t("brain_dump_and_sort.no_sessions")}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              {session.reflection && (
                <div className="mt-3 pt-3 border-t border-border">
                  <span className="text-xs text-muted-foreground">{t("brain_dump_and_sort.reflection")}: </span>
