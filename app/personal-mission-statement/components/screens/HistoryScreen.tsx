...
const HistoryScreen = ({ onBack }: HistoryScreenProps) => {
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            await dbRequest("DELETE FROM missions WHERE id = $1 AND user_id = $2", [id, userId]);
        } catch (err) {
        ...
        <ScreenWrapper screenKey="history">
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <button
                                        ...
                                        aria-label={t("personal_mission_statement.history_delete_aria")}
                                    >
                                        <Trash2 size={16} />
                                    </button>
                                </div>
            ...
            <div className="pt-8 pb-4">
                <MissionButton variant="outline" onClick={onBack}>
                    {t("personal_mission_statement.history_back")}
                </MissionButton>
            </div>
        </ScreenWrapper>
    );
