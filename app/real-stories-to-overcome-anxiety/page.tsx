...
import { BookOpen, Sparkles, ChevronRight, ChevronLeft, ArrowLeft, Heart, CheckCircle2 } from 'lucide-react';
...
function RealStoriesInner() {
  const { t } = useTranslation(undefined, { i18n });
  const [screen, setScreen] = useState<'intro' | 'selection' | 'story'>('intro');
  ...
  const handleNextStory = () => {
    if (selectedStoryIdx < STORIES.length - 1) {
      setSelectedStoryIdx((s) => s + 1);
    } else {
      setScreen('selection');
    }
  };
  ...
  const isLast = selectedStoryIdx === STORIES.length - 1;

  return (
    <PremiumLayout
      title={t('app_title', 'Inspirational Stories')}
      icon={<BookOpen className="w-6 h-6 text-primary animate-pulse" />}
      onBack={screen !== 'intro' ? handleBackToSelection : undefined}
    ...
    >
      <div className="w-full max-w-md mx-auto flex flex-col px-6 py-4 min-h-[70vh] text-center">
        <AnimatePresence mode="wait">
          ...
          {screen === 'intro' && (
            ...
            >
              <PremiumIntro
                ...
                description={t(
                  'app_description',
                  'Read authentic accounts of individuals who faced and overcame severe anxiety using courage and mindfulness.'
                )}
                onStart={handleStartStories}
                icon={<BookOpen size={32} />}
                benefits={[
                  t('intro_p1', 'Learn simple strategies for daily anxiety relief'),
                  t('intro_p2', 'Understand that you are not alone in your struggle'),
                ]}
          ...
          {/* SCREEN 3: INDIVIDUAL STORY VIEWER */}
