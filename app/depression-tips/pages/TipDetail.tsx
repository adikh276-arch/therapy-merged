...
import { tips } from "@/app/depression-tips/data/tips";
  ...
  if (!tip) {
    return (
      <div className="min-h-screen gradient-bg flex items-center justify-center">
        <p className="text-muted-foreground">{t("depression_tips.tipNotFound")}</p>
      </div>
    );
  }
      ...
      <div className="mx-auto max-w-md px-5 py-8 pb-16">
        ...
        <section className="mb-6 animate-fade-in-up" style={{ opacity: 0, animationDelay: "160ms" }}>
          <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-3">
            {t("depression_tips.whatYouCanDo")}
          </h2>
        ...
        {tip.example && (
          <section className="mb-6 animate-fade-in-up" style={{ opacity: 0, animationDelay: "240ms" }}>
            <h2 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">
              {t("depression_tips.example")}
            </h2>
            <div className="rounded-lg bg-card p-4 shadow-card space-y-2">
              <p className="text-sm text-muted-foreground">
                <span className="font-semibold">{t("depression_tips.insteadOf")}</span>{" "}
                {k ? t(`${k}_ex_instead`) : tip.example.instead}
              </p>
              <p className="text-sm text-foreground">
                <span className="font-semibold">{t("depression_tips.try")}</span>{" "}
