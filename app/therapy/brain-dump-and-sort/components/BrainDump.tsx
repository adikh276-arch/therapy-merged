  ...
  return (
    <div className="max-w-md mx-auto px-6 py-8 min-h-screen flex flex-col">
      <div className="mb-8 animate-fade-in">
        ...
        <p className="text-muted-foreground text-sm">{t("brain_dump_and_sort.dump_hint")}</p>
      </div>
      ...
      <div className="flex-1 mb-6 animate-fade-in" style={{ animationDelay: "0.1s" }}>
          ...
          {thoughts.map((thought, index) => (
            <div key={index} className="flex items-center gap-2">
              <input
                ...
                placeholder={index === 0 ? t("brain_dump_and_sort.dump_placeholder") : "..."}
        ...
        >
          + {t("brain_dump_and_sort.keep_going")}
        </button>

        <p className="text-xs text-muted-foreground/80">{t("brain_dump_and_sort.breathe")}</p>
      </div>
      ...
      >
        {t("brain_dump_and_sort.finished")}
      </button>
    </div>
  );
};
