...
import SelectableTile from "@/app/therapy/what-are-your-habits/components/SelectableTile";
import ActivityButton from "@/app/therapy/what-are-your-habits/components/ActivityButton";
import StepProgress from "@/app/therapy/what-are-your-habits/components/StepProgress";
import { useTranslation } from "react-i18next";

const CopingHabitsScreen = ({ onNext }: { onNext: () => void }) => {
  const { t } = useTranslation();
  const options = t("what_are_your_habits.coping_options", { returnObjects: true }) as string[];
  ...
  return (
    <div className="bg-card rounded-2xl p-6 shadow-sm border border-border flex flex-col gap-5">
      ...
      <h2 className="text-xl font-semibold text-foreground">{t("what_are_your_habits.coping_title")}</h2>
