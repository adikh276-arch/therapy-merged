...
import FloatingShapes from "@/app/therapy/doodle-burst/components/FloatingShapes";
import DrawingCanvas, { type DrawingCanvasRef } from "@/app/therapy/doodle-burst/components/DrawingCanvas";
import ShareModal from "@/app/therapy/doodle-burst/components/ShareModal";
import { saveDoodle } from "@/app/therapy/doodle-burst/lib/doodleHistory";
import { Sparkles, ArrowRight, Rocket, History, Share2 } from "lucide-react";
import { shareDoodle } from "@/app/therapy/doodle-burst/lib/share";
import { useTranslation } from "react-i18next";
import LanguageSelector from "@/app/therapy/doodle-burst/components/LanguageSelector";
...
const Index = () => {
  ...
  const PROMPTS = [
    { time: 60, text: t("doodle_burst.prompt_1") },
    { time: 30, text: t("doodle_burst.prompt_2") },
    { time: 10, text: t("doodle_burst.prompt_3") },
  ];
  ...
  const startActivity = useCallback(() => {
    setTimer(60);
    setCurrentPrompt(t("doodle_burst.prompt_1"));
    setScreen("activity");
  }, [t]);
        ...
        {screen === "intro" && (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center gap-6 py-12">
              ...
              >
                <History size={18} />
                {t("doodle_burst.view_past_doodles")}
              </motion.button>
            </div>
          </motion.div>
        )}
        ...
        {screen === "activity" && (
          ...
          >
            ...
            <p className="text-sm text-muted-foreground text-center leading-relaxed max-w-sm">
              {t("doodle_burst.activity_instructions")}
            </p>
        ...
        {screen === "end" && (
          ...
          >
            ...
            <h1 className="text-3xl font-black text-foreground text-center">
              {t("doodle_burst.end_title")}
            </h1>

            <p className="text-sm text-muted-foreground">{t("doodle_burst.end_saved")}</p>

            <div className="bg-card/80 backdrop-blur-sm rounded-3xl p-6 shadow-soft">
              <p className="text-foreground mb-4 text-justify">
                {t("doodle_burst.end_reset")}
              </p>
              <div className="flex flex-col gap-3 text-left">
                <CheckInItem emoji="🧠" text={t("doodle_burst.checkin_brain")} />
                <CheckInItem emoji="😌" text={t("doodle_burst.checkin_calmer")} />
                <CheckInItem emoji="🎯" text={t("doodle_burst.checkin_task")} />
              </div>
            </div>
