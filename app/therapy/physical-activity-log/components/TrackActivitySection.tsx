...
import { cn } from "@/app/therapy/physical-activity-log/lib/utils";
import { Button } from "@/app/therapy/physical-activity-log/components/ui/button";
import { Calendar } from "@/app/therapy/physical-activity-log/components/ui/calendar";
import { Popover, PopoverContent, PopoverTrigger } from "@/app/therapy/physical-activity-log/components/ui/popover";
import { useActivities, type ViewMode } from "@/app/therapy/physical-activity-log/hooks/useActivities";
import {
      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto mb-8 md:mb-12">
          ...
          <div className="wellness-card space-y-3 md:space-y-4">
            <h3 className="font-serif text-lg md:text-xl font-semibold text-foreground">{t("physical_activity_log.activity_log")}</h3>
            <div className="space-y-3">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {date ? format(date, "PPP") : <span>📅 {t("physical_activity_log.pick_date")}</span>}
                  </Button>
                </PopoverTrigger>
              ...
              <input
                type="text"
                placeholder={t("physical_activity_log.activity_placeholder")}
                value={activity}
              ...
              <input
                type="number"
                placeholder={t("physical_activity_log.duration_placeholder")}
                value={duration}
                onChange={(e) => setDuration(e.target.value)}
                className="w-full rounded-xl border border-input bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              />
              <textarea
                placeholder={t("physical_activity_log.notes_placeholder")}
                value={notes}
            ...
            <Button onClick={handleAdd} className="w-full rounded-xl h-10 md:h-11 font-semibold">
              {t("physical_activity_log.save_activity")}
            </Button>
          </div>
          ...
          <div className="wellness-card-blue space-y-3 md:space-y-4">
            <h3 className="font-serif text-lg md:text-xl font-semibold text-accent-foreground">
              📈 {t("physical_activity_log.progress_summary")}
            </h3>
            <div className="grid grid-cols-2 gap-3">
              <StatBox label={t("physical_activity_log.this_week")} value={`${stats.weekMinutes} min`} />
              <StatBox label={t("physical_activity_log.this_month")} value={`${stats.monthMinutes} min`} />
              <StatBox label={t("physical_activity_log.most_frequent")} value={stats.mostFrequent} />
              <StatBox label={t("physical_activity_log.longest_session")} value={stats.longestSession > 0 ? `${stats.longestSession} min` : "—"} />
            </div>
            <div className="pt-3 border-t border-border/50 flex justify-between items-center">
              <span className="text-sm font-medium text-foreground">🔥 {t("physical_activity_log.current_streak")}</span>
              <span className="text-xl md:text-2xl font-bold text-primary">{stats.streak} {stats.streak === 1 ? t("physical_activity_log.day") : t("physical_activity_log.days")}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto mb-8 md:mb-12">
          <div className="wellness-card">
            <h4 className="font-serif text-base md:text-lg font-semibold text-foreground mb-4">{t("physical_activity_log.last_7_days")}</h4>
          ...
          <div className="wellness-card">
            <h4 className="font-serif text-base md:text-lg font-semibold text-foreground mb-4">{t("physical_activity_log.weekly_trend")}</h4>
            <ResponsiveContainer width="100%" height={180}>
        ...
        <div className="max-w-4xl mx-auto">
          <h3 className="font-serif text-xl md:text-2xl font-bold text-foreground mb-4">📅 {t("physical_activity_log.activity_history")}</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-xl text-xs md:text-sm">
                  ...
                  {filterDate ? format(filterDate, "PPP") : t("physical_activity_log.filter_by_date")}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
            ...
            {filterDate && (
              <Button variant="ghost" size="sm" onClick={() => setFilterDate(undefined)} className="text-xs md:text-sm">
                {t("physical_activity_log.clear_filter")}
              </Button>
            )}
          </div>
          ...
          <div className="space-y-3">
            {filteredDates.length === 0 && (
              <div className="wellness-card-blue text-center py-8">
                <p className="text-muted-foreground">{t("physical_activity_log.no_activities")}</p>
              </div>
            )}
                  ...
                  {isExpanded && (
                    <div className="mt-3 space-y-2 pt-3 border-t border-border/30">
                        ...
                        <div key={a.id} className="flex items-center justify-between gap-2">
                          {editingId === a.id ? (
                            <div className="flex-1 flex gap-2 items-center flex-wrap">
                              ...
                              />
                              <Button size="sm" onClick={saveEdit} className="h-7 text-xs rounded-lg">{t("physical_activity_log.save")}</Button>
                              <Button size="sm" variant="ghost" onClick={() => setEditingId(null)} className="h-7 text-xs">{t("physical_activity_log.cancel")}</Button>
                            </div>
                          ) : (
                            <>
                      ...
                      <div className="pt-2 border-t border-border/30 flex justify-between">
                        <span className="text-xs font-medium text-foreground">{t("physical_activity_log.daily_total")}</span>
                        <span className="text-sm font-bold text-primary">{dayTotal} {t("physical_activity_log.minutes")}</span>
                      </div>
                    </div>
                  )}
                </div>
