...
const ChipSelect = ({
  ...
  <div className="flex flex-wrap gap-2">
    ...
    <input
      type="text"
      placeholder={t("thought_shifts.other")}
      value={otherValue}
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="relative bg-card/70 backdrop-blur-sm rounded-3xl p-8 border border-border/50 shadow-sm space-y-6">
                ...
                <motion.div variants={staggerChildren} initial="initial" animate="animate" className="space-y-4 text-foreground/80 leading-relaxed text-[15px]">
                  <motion.p variants={childFade}>
                    {t("thought_shifts.intro_text_1")}<br />
                    {t("thought_shifts.intro_text_2")}
                  </motion.p>
                  <motion.p variants={childFade}>{t("thought_shifts.influence_title")}</motion.p>
                  <motion.ul variants={childFade} className="space-y-2 ml-1">
                    <li className="flex items-center gap-3">
                      <span className="w-2 h-2 rounded-full bg-pastel-blue flex-shrink-0" />{t("thought_shifts.how_we_feel")}
                    </li>
                    <li className="flex items-center gap-3">
                      <span className="w-2 h-2 rounded-full bg-pastel-lavender flex-shrink-0" />{t("thought_shifts.what_we_do")}
                    </li>
                    <li className="flex items-center gap-3">
                      <span className="w-2 h-2 rounded-full bg-pastel-mint flex-shrink-0" />{t("thought_shifts.avoid_action")}
                    </li>
                  </motion.ul>
                  <motion.p variants={childFade}>{t("thought_shifts.help_you_title")}</motion.p>
                  <motion.div variants={childFade} className="space-y-2 ml-1">
                    <div className="flex items-center gap-3">
                      ...
                      {t("thought_shifts.notice_thought")}
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {t("thought_shifts.understand_impact")}
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {t("thought_shifts.adjust_balanced")}
                    </div>
                  </motion.div>
                  <motion.p variants={childFade} className="italic text-muted-foreground border-l-2 border-pastel-lavender pl-4">
                    {t("thought_shifts.not_forcing_positivity")}<br />
                    {t("thought_shifts.gaining_clarity")}
                  </motion.p>
                  <motion.p variants={childFade} className="text-sm text-muted-foreground">
                    {t("thought_shifts.time_required")}<br />
                    {t("thought_shifts.no_right_wrong")}
                  </motion.p>
                </motion.div>
                <CTAButton onClick={next}>{t("thought_shifts.start")}</CTAButton>
              </div>
            </motion.div>
          )}
          ...
          {screen === 1 && (
            <motion.div key="s1" {...fadeVariants} className="relative">
              ...
              <div className="relative bg-card/70 backdrop-blur-sm rounded-3xl p-8 border border-border/50 shadow-sm space-y-6">
                <h1 className="text-3xl font-heading text-foreground">{t("thought_shifts.whats_the_thought")}</h1>
                <div className="space-y-4 text-foreground/80 leading-relaxed text-[15px]">
                  <p>{t("thought_shifts.thought_bothering")}</p>
                  <p>
                    {t("thought_shifts.self_critical")}<br />
                    {t("thought_shifts.overwhelming")}<br />
                    {t("thought_shifts.completely_true")}
                  </p>
                  <p>{t("thought_shifts.write_one_sentence")}</p>
                </div>
                <div className="space-y-2">
                  <input
                    ...
                    placeholder={t("thought_shifts.thought_placeholder")}
                  ...
                  />
                  <p className="text-xs text-muted-foreground ml-2">{t("thought_shifts.no_judgment")}</p>
                </div>
                <CTAButton onClick={next} disabled={!thought.trim()}>{t("thought_shifts.continue")}</CTAButton>
              </div>
            </motion.div>
          )}
          ...
          {screen === 2 && (
            <motion.div key="s2" {...fadeVariants} className="relative">
              ...
              <div className="relative bg-card/70 backdrop-blur-sm rounded-3xl p-8 border border-border/50 shadow-sm space-y-6">
                <h1 className="text-3xl font-heading text-foreground">{t("thought_shifts.affect_title")}</h1>
                <p className="text-foreground/80 text-[15px]">{t("thought_shifts.when_you_think")}</p>
                <div className="space-y-3">
                  <p className="text-sm font-medium text-foreground">{t("thought_shifts.feel_question")}</p>
                  <ChipSelect
                    options={feelings}
                ...
                <div className="space-y-3">
                  <p className="text-sm font-medium text-foreground">{t("thought_shifts.do_question")}</p>
                  <ChipSelect
                    options={actions}
                ...
                <div className="bg-pastel-blue/30 rounded-2xl p-5 space-y-1.5 border border-pastel-blue/40">
                  <p className="text-sm font-medium text-accent-foreground">{t("thought_shifts.pattern_title")}</p>
                  <p className="text-xs text-muted-foreground">
                    {t("thought_shifts.automatic_pattern")}<br />
                    {t("thought_shifts.awareness_space")}
                  </p>
                </div>
                <CTAButton onClick={next} disabled={selectedFeelings.length === 0 && !feelingOther}>{t("thought_shifts.continue")}</CTAButton>
              </div>
            </motion.div>
          )}
          ...
          {screen === 3 && (
            <motion.div key="s3" {...fadeVariants} className="relative">
              ...
              <div className="relative bg-card/70 backdrop-blur-sm rounded-3xl p-8 border border-border/50 shadow-sm space-y-6">
                <h1 className="text-3xl font-heading text-foreground">{t("thought_shifts.supports_title")}</h1>
                <div className="space-y-4 text-foreground/80 leading-relaxed text-[15px]">
                  <p>{t("thought_shifts.supports_text_1")}</p>
                  <p>{t("thought_shifts.supports_text_2")}</p>
                  <p>{t("thought_shifts.ask_yourself")}</p>
                  <p className="italic text-foreground/70 border-l-2 border-pastel-peach pl-4">
                    {t("thought_shifts.valid_question")}
                  </p>
                  <p>
                    {t("thought_shifts.no_prove_wrong")}<br />
                    {t("thought_shifts.gently_explore")}
                  </p>
                </div>
                <div className="space-y-2">
                  <input
                    ...
                    placeholder={t("thought_shifts.evidence_placeholder")}
                    className="w-full px-5 py-4 rounded-2xl border-2 border-border bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 text-[15px] transition-all"
                  />
                  <p className="text-xs text-muted-foreground ml-2">{t("thought_shifts.be_specific")}</p>
                </div>
                <div className="bg-pastel-peach/30 rounded-2xl p-4 border border-pastel-peach/40">
                  <p className="text-xs text-muted-foreground">
                    {t("thought_shifts.evidence_logic")}
                  </p>
                </div>
                <CTAButton onClick={next} disabled={!evidence.trim()}>{t("thought_shifts.reframe")}</CTAButton>
              </div>
            </motion.div>
          )}
          ...
          {screen === 4 && (
            <motion.div key="s4" {...fadeVariants} className="relative">
              ...
              {!finished ? (
                <div className="relative bg-card/70 backdrop-blur-sm rounded-3xl p-8 border border-border/50 shadow-sm space-y-6">
                  <h1 className="text-3xl font-heading text-foreground">{t("thought_shifts.adjust_title")}</h1>
                  <div className="space-y-3 text-foreground/80 leading-relaxed text-[15px]">
                    <p>{t("thought_shifts.adjust_intro")}</p>
                    <p>
                      {t("thought_shifts.not_overly_positive")}<br />
                      {t("thought_shifts.realistic_fair")}
                    </p>
                    <p>{t("thought_shifts.balanced_often")}</p>
                    <ul className="space-y-2 ml-1">
                      <li className="flex items-center gap-3">
                        <span className="w-2 h-2 rounded-full bg-pastel-blue flex-shrink-0" />{t("thought_shifts.acknowledges_true")}
                      </li>
                      <li className="flex items-center gap-3">
                        <span className="w-2 h-2 rounded-full bg-pastel-lavender flex-shrink-0" />{t("thought_shifts.leaves_room")}
                      </li>
                      <li className="flex items-center gap-3">
                        <span className="w-2 h-2 rounded-full bg-pastel-mint flex-shrink-0" />{t("thought_shifts.reduces_harsh")}
                      </li>
                    </ul>
                  </div>
                  <div className="space-y-2">
                    <input
                      ...
                      placeholder={t("thought_shifts.reframe_placeholder")}
                      className="w-full px-5 py-4 rounded-2xl border-2 border-border bg-background text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 text-[15px] transition-all"
                    />
                    <p className="text-xs text-muted-foreground ml-2">
                      {t("thought_shifts.reframe_hint")}<br />
                      {t("thought_shifts.reframe_hint_2")}
                    </p>
                  </div>
                  <CTAButton onClick={() => setFinished(true)} disabled={!reframe.trim()}>{t("thought_shifts.submit")}</CTAButton>
                </div>
              ) : (
                <motion.div
                ...
                >
                  ...
                  <div className="relative bg-card/80 backdrop-blur-sm rounded-3xl border border-border/50 shadow-lg overflow-hidden">
                    ...
                    <div className="p-8 space-y-6">
                      ...
                      >
                        <motion.p variants={childFade}>
                          {t("thought_shifts.still_exist")}<br />
                          {t("thought_shifts.no_eliminate")}
                        </motion.p>

                        <motion.p variants={childFade} className="font-medium text-foreground text-base">
                          {t("thought_shifts.powerful_practice")}
                        </motion.p>

                        <motion.div variants={childFade} className="space-y-3 py-2">
                          {[
                            { color: "bg-pastel-blue", text: t("thought_shifts.paused") },
                            { color: "bg-pastel-lavender", text: t("thought_shifts.looked_thought") },
                            { color: "bg-pastel-mint", text: t("thought_shifts.explored_supports") },
                            { color: "bg-pastel-peach", text: t("thought_shifts.understood_affect") },
                            { color: "bg-pastel-rose", text: t("thought_shifts.chose_respond") },
                          ].map(({ color, text }, i) => (
                            <motion.div
                              key={i}
                        ...
                        >
                          <p className="font-medium text-foreground">{t("thought_shifts.creates_space")}</p>
                          <p className="text-sm">
                            {t("thought_shifts.small_moments")}
                          </p>
                        </motion.div>
                        ...
                        >
                          <p>
                            {t("thought_shifts.not_change_every")}<br />
                            {t("thought_shifts.only_notice")}
                          </p>
                          <p className="text-xl font-heading text-primary">{t("thought_shifts.did_today")}</p>
                        </motion.div>
                      </motion.div>
                      ...
                      >
                        <CTAButton onClick={reset} variant="soft">{t("thought_shifts.finish")}</CTAButton>
                      </motion.div>
                    </div>
                  </div>
                </motion.div>
